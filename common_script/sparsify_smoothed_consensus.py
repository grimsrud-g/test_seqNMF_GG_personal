...
def main():
    ...
    W_NKL = np.transpose(W, axes=(N_ax, K_ax, L_ax))  # shape (N, L, K)
    N_old, K, L = W_NKL.shape
    
    # now we need to do our reconstruction walk-through: first make W-full w/r/t original fsaverage4
    W_full = np.zeros((full_length, K, L), dtype=W.dtype)
    
    # now port in group-level fit
    W_full[old_kept_indices, :, :] = W_NKL
    
    # ===== NEW: SMOOTH ON SURFACE =====
    ...
    W_NKL = W_full_smoothed[new_kept_indices, :, :]
    
    # compute 50th percentile (median) across N for each (L, K)
    thresh = np.median(W_NKL, axis=0, keepdims=True)  # shape (1, L, K)
    ...
    print(f"Saved smoothed+pruned W to: {out_path}")
