...
import sys
sys.path.append('..')
# A.P - 11/1/25 change to _rf for refit version of jseqNMF with 0-multiplied group-to-individual spatial enforcement
from .models_rf import jSeqNMF
...
def _load_template(template_npz):
    """
    Load W from a template .npz (previous fit).
    Returns W_template.
    """
    # "checkpoint", but really an established group consensus run
    ckpt = np.load(template_npz)
    if 'W' not in ckpt:
        raise ValueError(f"{template_npz} missing key 'W'")
    W_t = ckpt['W']
    return W_t
...
def refit_jNMF(data_loader, result_file, K, L, lam, lambda_OrthH, lambda_OrthW, lambda_L1W, lambda_L1H, nH, seed, num_iters, device, dtype=torch.float32, no_improve_tol=50, W_fixed=False, W=None, renorm_type='renorm',template_npz=None):
    assert ((not W_fixed) and (W is None)) or (W_fixed and (W is not None)), "W must be provided if W_fixed is True"
    if (W is None) and (template_npz is not None):
        print('Re-fitting on init W')
        W = _load_template(template_npz).astype(np.float32)

    # make sure template_npz is fed in
    if template_npz is None:
        raise ValueError("template_npz is required for refitting")

    with torch.no_grad():
        torch.manual_seed(seed)
        np.random.seed(seed)
        # check for extant results file
        best_result_file = _best_checkpoint_path(result_file)
     
        # this seems to be a try/catch to load in the results file and ensure it hasn't already ran as many times as it needs to  
        try:
            ...
            # is the current iteration any better?
            ...
            # if no extant results file, start with template file
            W_t = _load_template(template_npz)
            model = jSeqNMF(
                K=K, L=L, lam=lam, nH=nH, device=device,
                lambda_OrthH=lambda_OrthH, lambda_OrthW=lambda_OrthW,
                lambda_L1H=lambda_L1H, lambda_L1W=lambda_L1W, 
                W_init=W_t, W_fixed=W_fixed,
                dtype=dtype
            )
            # initialize values for iterations
        ...
        # initialize numerator and denom for multiplicative updates?
