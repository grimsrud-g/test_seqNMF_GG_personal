...
# quick and dirty fitting adjustment to enforce spatial expanse of components K in group-to-individual fitting - AP 11/1/25
...
class SeqNMF(nn.Module):
    ...
    def do_mult_update_step(self, X, Mt):
        ...
        self.shift_factors()
        # re-fit version (without fluctuating bottom 0's) - A.P. 11/1/25
        #self.W += smallnum * (self.W > 0).to(self.W.dtype)
        # this seems to work, but trying a version with very small smallnum addition for tests
        self.W += smallnum
